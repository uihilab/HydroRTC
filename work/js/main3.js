...
function receiveDataChromeFactory() {
  var buf = "", count = 0, byteLength;

  return function onmessage(event) {
    if (typeof event.data === 'string' && count == 0) {
      byteLength = window.buf = parseInt(event.data);
      count += 1;
    ...
    if (buf.length >= byteLength) {
        ...
        console.log('Done. Rendering File.');
        renderFile(buf);
    }
  }
};
...
document.querySelector("input[type=file]").onchange = function (event) {
    ...
    fileReader.onload = function(fileLoadedEvent){
        ...
        uploadFileText.value = textFromFileLoaded;
    };
...
function sendFile() {
    ...
    let len = uploadFileText.value.length,
    n = len / CHUNK_LEN | 0;
    ...
    let data = uploadFileText.value
    // split the photo and send in chunks of about 64KB
    for (var i = 0; i < n; i++) {
    var start = i * CHUNK_LEN,
    end = (i + 1) * CHUNK_LEN;
    console.log(start + ' - ' + (end - 1));
    dataChannel.send(data.substring(start, end));
    }

    // send the reminder, if any
    if (len % CHUNK_LEN) {
    console.log('last ' + len % CHUNK_LEN + ' byte(s)');
    dataChannel.send(data.substring(n * CHUNK_LEN));
    }
}
